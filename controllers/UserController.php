...
use app\models\ImportDokterBerau;
...
class UserController extends Controller
{
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($user = $model->signup()) {
                ...
                if($Dokter->save()) $transaction->commit();
                    ...
                    \Yii::$app->getSession()->setFlash('danger', 'Gagal Menambahkan Pengguna');
                ...
                \Yii::$app->getSession()->setFlash('success', 'Berhasil Menambahkan Pengguna');
    ...
    public function actionImportPenggunaBerau()
    {
        set_time_limit(0);
        $data = ImportDokterBerau::find()->asArray()->all();
        $transaction = ImportDokterBerau::getDb()->beginTransaction(); 

        foreach ($data as $key => $value) {
            $model = new SignupForm();
            $model->username = $value['nama_dokter'];
            $model->email = "dokter$key@rsberau.rs";
            $model->password = $value['password_default'];
            $model->klinik_id = 109;
            $model->role = 20;
            $user = $model->signup();
