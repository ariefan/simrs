...
class DiagnosisController extends Controller
{
            ...
            'access' => [
                   ...
                   'rules' => [
                       [
                           ...
                           'allow' => true,
                           'roles' => [
                               User::ROLE_ADMIN, User::ROLE_CODING
                           ],
                       ]
                   ],
            ],
    ...
    public function actionCariProcedure($q=null)
    {
        \Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
        $out = ['results' => ['id' => '', 'text' => '']];
        if (!is_null($q)) {
            $query = new Query;
            $query->select('id,short_desc AS text')
                ->from('eklaim_icd9cm')
                ->where(['like', 'short_desc', $q])
                ->orWhere(['like', 'id', $q])
                ->orWhere(['like', 'long_desc', $q])
                ->limit(20);
            $command = $query->createCommand();
            $data = $command->queryAll();
            foreach ($data as $key => $value) {
