...
/**
 * RmRadController implements the CRUD actions for RmRad model.
 */
class RmRadController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    /**
     * Lists all RmRad models.
     * @return mixed
     */
    /*public function actionIndex()
    {
        $searchModel = new RmRadSearch();
    ...
    public function actionCreate($id)
    {
        ...
        $dokter = '';
        if(!empty($rm)){
            $data_rad = RmRad::find()->joinWith('jenis')->where(['rm_id'=>$rm['rm_id']])/*->asArray()*/->all();
            $rm_id = $rm['rm_id'];
            if(!empty($data_rad))
                $dokter = $data_rad[0]['dokter'];
        }

        $rm_rad = [];
        foreach ($data_rad as $key => $value) {
            $rm_rad[$key] = $value['medicalunit_cd'];
        }
        $data_dokter = Dokter::find()->joinWith('user')->where(['role'=>'20'])->orWhere(['role'=>'25'])->asArray()->all();
        $item_rad = UnitMedisItem::getWithSub('RADIO00');
        $post_data = Yii::$app->request->post();
        ...
        if (!empty($post_data)) {
            ...
            if(isset($post_data['tindakan']['listTindakan'])){
                ...
                foreach ($post_data['tindakan']['listTindakan'] as $key => $value) {
                    $ada[]=$value;
                    $RmRad = new RmRad();
                    $ex = RmRad::find()->where(['rm_id'=>$rm_id,'medicalunit_cd'=>$value])->asArray()->one();
                    if(!empty($ex))
                        continue;
                    $RmRad->rm_id = $rm_id;
                    $RmRad->medicalunit_cd = $value;
                    $d = Dokter::findOne($post_data['tindakan']['listDokter'][$key]);
                    $RmRad->dokter = $post_data['tindakan']['listDokter'][$key];
                    $RmRad->jumlah = $post_data['tindakan']['listJumlah'][$key];
                    $RmRad->dokter_nama = $d->nama;
                    $d = UnitMedisItem::findOne($value);
                    $RmRad->nama = $d['medicalunit_nm'];
                    $RmRad->save();
                }

                foreach ($rm_rad as $key => $medicalunit_cd) {
                    if(!in_array($medicalunit_cd, $ada)){
                        RmRad::deleteAll("rm_id=$rm_id AND medicalunit_cd='$medicalunit_cd'");
                    }
                }
            } else {
                RmRad::deleteAll("rm_id=$rm_id");
            }
                // echo '<pre>';
        ...
        // echo '<pre>';
        // print_r($data_rad);exit;
        return $this->render('create', compact('model','item_rad','pasien','data_dokter','rm_rad','data_rad','dokter'));
    ...
    //     $pasien = Pasien::findOne($model->mr);
    //     $data_lab = RmRad::find()->joinWith('jenis')->where(['rm_id'=>$id])->asArray()->all();
    ...
    public function actionUnduh($id)
    {
        ...
        $SQL = "SELECT DISTINCT a.id, a.rm_id as 'No Rekam Medis',b.nama as 'Nama Pasien',c.tanggal_periksa as 'Tanggal Periksa',d.medunit_nm as 'Unit Pengirim',f.nama as 'Dokter Pengirim',e.medicalunit_nm, a.catatan as 'Catatan', a.hasil as 'Hasil' FROM rm_rad a, pasien b, kunjungan c, unit_medis d, unit_medis_item e, dokter f, rekam_medis g WHERE (a.rm_id=g.rm_id) and (g.kunjungan_id=c.kunjungan_id) and (g.user_id=f.user_id) and (g.mr=b.mr) and (d.medunit_cd=c.medunit_cd) and (e.medicalunit_cd=a.medicalunit_cd) and (a.id=$id) GROUP BY a.id ";        
        $hasil = RmRad::findBySql($SQL)->asArray()->all();
        ...
        $pdf = new Pdf([
            ...
            'filename' => "Rad".$model->medicalunit_cd."_idPeriksa".$model->id."_"."NoRM ".$model->rm_id.".pdf",
            // any css to be embedded if required
            'cssInline' => '.kv-heading-1{font-size:18px}', 
             // set mPDF properties on the fly
            'options' => ['title' => "Pemeriksaan Radiologi ".$model->id." ".$model->rm_id],
    ...
    /**
     * Updates an existing RmRad model.
    ...
    /**
     * Deletes an existing RmRad model.
    ...
    /**
     * Finds the RmRad model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param string $id
     * @return RmRad the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = RmRad::findOne($id)) !== null) {
            return $model;
        } else {
            return null;
        }
