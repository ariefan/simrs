        ...
        $('#cari-obat').keyup(function(){
            ...
            $.post('{$urlCari}',{keyword:$(this).val()})
	            .done(function(data){
	              ...
	              str_hasil = ""
	              for(var i in data){
	                str_hasil += "<tr><td>"+data[i].item_nm+"</td><td>"+data[i].unit_cd+"</td><td>"+data[i].type_nm+"</td><td><button type='button' class='pilih-obat btn btn-primary' item_cd="+data[i].item_cd+" unit_cd="+data[i].unit_cd+" item_nm='"+data[i].item_nm+"'>Pilih</button>  </td></tr>"
	              }
