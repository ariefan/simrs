...
<div class="tarif-paramedis-form">

    <?php $form = ActiveForm::begin(); ?>

    <?php 
        $model->tarif_paramedis_id = $model->idInsert; 

        echo $form->field($model, 'tarif_paramedis_id')->textInput(['maxlength' => true]);
    ?>
    ...
    <?= $form->field($model, 'specialis_cd')->widget(Select2::classname(), [
        'data' =>  ArrayHelper::map(Spesialis::find()->select(['spesialis_id', 'nama'])->asArray()->all(), 'spesialis_id', 'nama'),
        'options' => ['placeholder' => 'specialis_cd'],
        'pluginOptions' => [
            'allowClear' => true
        ],
    ]); ?>

    <?= $form->field($model, 'paramedis_tp')->textInput(['maxlength' => true]) ?>
