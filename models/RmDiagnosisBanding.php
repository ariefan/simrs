...
class RmDiagnosisBanding extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            [['rm_id'], 'integer'],
            [['kode'], 'string', 'max' => 15],
            [['nama_diagnosis','kasus'],'string','max'=>255],
    ...
    public function kasusBaruLama($rm_id, $kode=null)
    {
        if($this->kode!=null)
            $kode = $this->kode;
        elseif($kode==null && $this->kode=='')
            return false;

        $mr = RekamMedis::findOne($rm_id)->mr;

        $pernahMengidap = RekamMedis::find()->leftJoin('rm_diagnosis_banding','rm_diagnosis_banding.rm_id = rekam_medis.rm_id')->where(['mr'=>$mr, 'kode'=>$kode])->count();

        return ($pernahMengidap > 0)? "Lama":"Baru";
    }
}
